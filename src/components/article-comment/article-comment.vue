<template>
  <div class="article-comment" @click="showComment">
    <svg width="24" height="24" viewBox="0 0 24 24" aria-label="responses" class="button-icon">
      <path
    ...
    <div class="commnet-total">{{ commentTotal }}</div>
  </div>
  <fe-drawer v-model="visible" placement="right">
    <comment-close-button @click="showComment"></comment-close-button>
    <comment-input @click-modal="modalHandler"></comment-input>
...
import CommentCloseButton from './comment-close-button.vue'
...
export default defineComponent({
  ...
  emits: ['click-textarea'],
  setup(props, { emit }) {
    const visible = ref(false)
    const commentTotal = computed(() => CommentStore.commentTotal)
    const showComment = () => {
